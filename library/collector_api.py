...
class collector_api():
    ...
    def variable_setting(self):
        self.open_api.py_gubun = "collector" # open_api 에서는 False가 디폴트값
        self.dc = daily_crawler(self.open_api.cf.real_db_name, self.open_api.cf.real_daily_craw_db_name, #daily_buy_list 모듈에 import됨
    ...
    def code_update_check(self):
        ...
        if rows[0][1] != self.open_api.today or rows[0][2] != self.open_api.today:
            ...
            self.open_api.set_invest_unit() #매수 금액 설정하는 함수
    ...
    def realtime_daily_buy_list_check(self):
        if self.open_api.sf.is_date_exist(self.open_api.today):
            ...
            self.open_api.sf.get_date_for_simul()  #시뮬레이터 시작,끝날짜 장열린 날들가져오는함수
            ...
            self.open_api.sf.update_all_db_by_date(self.open_api.today)  # 보유 중인 종목들의 주가를 일별로 업데이트 하는 함수
            self.open_api.rate_check() # all_item_db의 rate를 업데이트 한다.
            # realtime_daily_buy_list(매수 리스트) 테이블 세팅을 완료 했으면 아래 쿼리를 통해 setting_data의 today_buy_list에 오늘 날짜를 찍는다.
            sql = "UPDATE setting_data SET today_buy_list='%s' limit 1" #limit 1의 의미는?!
    ...
    def set_daily_crawler_table(self, code, code_name):
        df = self.open_api.get_total_data(code, code_name, self.open_api.today) #df에 받아오는 데이터가없음 빈리스트반환
        print(df)
